...
IMAGE_SIZE    = 512
TIMESTEPS     = 1000
T_START       = 250       # steps used for reconstruction
T_EXPLAIN     = 1000      # full steps for explainability visuals
EXPLAIN_STEPS = [999, 750, 500, 250, 0]  # captured within 0-999
PATCH_SIZE    = 64
FINAL_SIZE    = 1024
NUM_IMAGES    = 5
...
# -------- RECONSTRUCTION (SDEdit, T_START=250) --------
@torch.no_grad()
...
def evaluate(corrupt_dir, tag):
    ...
    for idx in range(min(NUM_IMAGES, len(dataset))):
        ...
        # save matching HQ at 512px for fair FID comparison
...
# -------- EXPLAINABILITY --------
def save_row(images, labels, filename):
    n      = len(images)
    margin = 100
    canvas = Image.new("RGB", (IMAGE_SIZE * n + margin, IMAGE_SIZE + 60), (255, 255, 255))
    ...
    for i, lbl in enumerate(labels):
        draw.text(
            (margin + i * IMAGE_SIZE + IMAGE_SIZE // 3, IMAGE_SIZE + 10),
            f"t={lbl}", fill=(0, 0, 0), font=font
        )
...
def generate_explainability(dataset, tag):
    # always use first image for consistent comparison across epochs
    cond, _, _ = dataset[0]
    cond = cond[None].to(DEVICE)

    # ✅ full 1000-step noising for explainability
    x = diffusion.q_sample(
        cond,
    ...
    # ✅ loop full 1000 steps: 999 down to 0
    for t in reversed(range(T_EXPLAIN)):
        ...
        # ✅ capture at t=999, 750, 500, 250, 0
    ...
    # reverse so left=noisy(t=999), right=clean(t=0)
    traj.reverse()
    score_maps.reverse()
    patches.reverse()
    labels = EXPLAIN_STEPS[::-1]   # [999, 750, 500, 250, 0]
